...
所需信息：
	- 定义：像素点，代表屏幕上最小的发光单位，像素点可以被绘制成各种颜色。

	- 定义：方块，表示蛇的一节身体、头、食物，所有的方块
		由像素点组成。

	- 定义：蛇身体之间的间隙，也是方块。

	- 定义：坐标，距离特定位置的距离，特定位置，可以是这个屏幕的左上角，或者当		前窗体的左上角。


	- 配置：窗体由很多像素点组成，横向w_width个，
		纵向w_height个，坐标为(xw,yw),颜色为空。

	- 配置：矩形边界，由像素点组成，横向为b_width个，纵向b_height个，
		颜色为蓝色。

	- 配置：蛇的身体和头由像素点组成，横向s_width个，纵向s_height个，
		坐标为(xs,ys),颜色为黑色。
		
	- 配置：食物由像素点组成，横向f_width个，纵向f_height个，坐标为(xf,yf),
		颜色为红色。
	- 
	- 配置：屏幕由。
	- 配置：定时器，每移动一格的间隔时间：1s(1000ms)

	- 常量：蛇的初始状态，蛇头一个方块，蛇身一个方块，食物一个方块。
...
- [ ] 绘制矩形边界。
	...
	调用setPen()设置画笔及颜色，调用drawRect()函数，绘制矩形边界770x420，距离窗体左上角，矩形左上角坐标为(10,10),蓝色线条。
...
- [ ] 定义一个蛇类，蛇类的结构如下
	...
	- [ ] 绘制蛇 
		...
		调用setBrush()设置画笔为画刷，颜色为默认的黑色,遍历整个list的元素，调用drawRect(list[i][0],list[i][1], 60, 60),将全部的矩形绘制出来。其中list[0]为蛇的头，其他元素的作为蛇的身体，list[-1]是蛇的最后一个元素。
	...
	- [ ] 撞到食物
		如果list[0][0]和list[0][1]的值，同时等于食物类的xfood, yfood,则表明蛇碰撞到食物。需要向表示蛇的list中加入一个元素，并将list[0]的值赋值给新元素。
		然后，将食物的坐标赋值给list[0]，同时改变画刷的颜色为蛇的颜色(黑色),重绘食物所在的矩形区域。方向值维持不变。
