...
当方向为0，则在数组首部插入新的元素，并将原来的数组首部增加D,然后赋值给新元素，则数组变为：
...
|  4  |  3  |  2  |  1  |
	...
	- 未发生碰撞，根据方向值，判断在当前移动方向下，如果蛇头的下一个位置是空白，则未发生碰撞，蛇继续移动；
		否则，如果下一个位置是食物、墙、蛇自身，则发生碰撞。

	- 撞墙：蛇头位置超出矩形边框，或者与矩形边框重合。

	- 撞到蛇自身：蛇头位置与蛇自身任意一节重合。

	- 撞到食物：如果在当前移动方向下，蛇头移动的下一个位置是食物，
		则蛇发生增长，蛇身变长1个方块，蛇按照碰撞前的方向继续移动，
		在其他位置生成新的食物。
...
方向为0，如果下一次移动将碰撞到食物，则向数组插入一个新元素，并将食物的数据存储到新元素中，则数组变为：
...
#####绘制信息：(View)
	...
	- 配置：食物由方格组成，食物不能与蛇重合，颜色为红色。

	- 常量：蛇初始状态，蛇头，蛇身各1个方块，黑色，特定位置；食物，用多个方格表示，大小和蛇身一致，红色，特定位置。
